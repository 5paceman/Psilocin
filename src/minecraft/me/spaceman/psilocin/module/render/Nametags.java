...
public class Nametags extends Module {
    ...
    public void onRenderNametags(final RenderNametagsEvent event)
    {
        ...
        double distance = event.getEntity().getDistanceSqToEntity(this.renderManager.livingPlayer);
        double distanceMod = distance / 15f;
        float scaleFactor = (float)Math.max(1, Math.min(1f, distanceMod));
        double x = event.getX();
        double y = event.getY() + 0.25;
        double z = event.getZ();

        String s = event.getEntity().getDisplayName().getFormattedText() +  " H:" + event.getEntity().getHealth();
